...
- Header: 서버 layout이 전달한 인증 여부로 회원 메뉴를 표시한다. Supabase Auth 변경 이벤트는 서버 재조회만 유발하고 클라이언트 이벤트의 user/session을 신뢰해 표시를 결정하지 않는다.
...
`node --test tests/signup.test.cjs tests/auth.test.cjs`는 API를 모의 처리한다. 로그인 성공/실패, 중복 요청, 로그아웃 실패 복구, 서버 보호, profile UUID 필터, callback 응답의 쿠키, 세션별 완료 문구, 외부 redirect 차단을 확인한다. 실제 서버가 발급한 토큰 및 이메일 링크 검증을 대체하지 않는다.
...
3. 마이페이지에서 로그아웃 → 홈 → Header 로그인/회원가입 → `/mypage` 재접근 차단.
