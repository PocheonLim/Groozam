...
export default async function MyPage({ searchParams }: { searchParams: Promise<{ section?: string }> }) {
  ...
  return <main className="mx-auto max-w-[1200px] px-5 py-12 md:px-10 md:py-20">
    ...
    <div className="mt-8 flex flex-col justify-between gap-6 bg-stone-50 p-6 sm:flex-row sm:items-center md:p-8">
      <div><h2 className="text-xl font-medium">그루잠에 오신 것을 환영합니다.</h2><p className="mt-3 text-sm leading-6 text-stone-600">로그인하고 주문 내역과 배송 정보를 한곳에서 확인하세요.</p><p className="mt-2 text-xs text-stone-500">회원 서비스 오픈 준비 중입니다.</p></div>
      <div className="flex shrink-0 gap-2"><Link href="/login" className="bg-stone-900 px-6 py-3 text-sm text-white">로그인</Link><Link href="/signup" className="border border-stone-300 bg-white px-6 py-3 text-sm">회원가입</Link></div>
    </div>
    ...
    <div className="mt-10 grid gap-8 md:grid-cols-[180px_1fr] md:gap-12">
      ...
      <section className="min-w-0" aria-label={current.label}>
        {current.key === "overview" ? <>
          ...
          <ol className="grid grid-cols-2 gap-y-6 border-b border-stone-200 py-8 sm:grid-cols-4">{["결제 완료", "상품 준비", "배송 중", "배송 완료"].map((step) => <li key={step} className="text-center"><span className="block text-2xl text-stone-300" aria-label="로그인 후 확인">—</span><span className="mt-3 block text-sm text-stone-600">{step}</span></li>)}</ol>
          <p className="py-8 text-center text-sm text-stone-500">로그인 후 주문 현황을 확인할 수 있습니다.</p>
          ...
          <dl className="divide-y divide-stone-200 border-b border-stone-200">{["이름", "이메일", "연락처", "연결된 소셜 계정"].map((label) => <div key={label} className="grid grid-cols-[120px_1fr] gap-4 py-5 text-sm"><dt className="text-stone-600">{label}</dt><dd className="text-stone-400">로그인 후 확인</dd></div>)}</dl>
          <LoginPrompt text="회원정보 변경과 계정 연결은 로그인 후 이용할 수 있습니다." />
        </> : <>
          <h2 className="border-b border-stone-900 pb-4 text-lg font-medium">{current.label}</h2>
          <LoginPrompt text={current.key === "addresses" ? "배송지 이름, 받는 분, 연락처와 주소를 등록하고 기본 배송지를 설정할 수 있습니다." : current.key === "claims" ? "취소·교환·반품 신청과 처리 상태를 확인할 수 있습니다." : "주문한 상품과 배송 진행 상황을 확인할 수 있습니다."} />
        </>}
      </section>
    </div>
  </main>;
...
function LoginPrompt({ text }: { text: string }) {
  return <div className="border-b border-stone-200 px-4 py-16 text-center"><p className="text-base">로그인이 필요합니다.</p><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-stone-500">{text}</p><Link href="/login" className="mt-6 inline-block border-b border-stone-900 pb-1 text-sm">로그인하기</Link></div>;
}
